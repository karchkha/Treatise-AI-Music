...
from .pann_model import create_pann_model, create_latent_pann_model
...
from clap.training.get_latent_data import FirstStageModel
...
class LOSS_CLAP(nn.Module):
    def __init__(
        ...
        if isinstance(audio_cfg, dict):
            audio_cfg = CLAPAudioCfp(**audio_cfg)
        if isinstance(text_cfg, dict):
            latent_cfg = CLAPLatentCfg(**text_cfg)
            # text_cfg = CLAPTextCfg(**text_cfg)

        self.audio_cfg = audio_cfg
        # self.text_cfg = text_cfg
        self.latent_cfg = latent_cfg
        self.enable_fusion = enable_fusion
        self.fusion_type = fusion_type
        self.joint_embed_shape = joint_embed_shape
        self.mlp_act = mlp_act
        self.use_fist_stage = use_fist_stage
        self.use_make_noisy = use_make_noisy

        if self.use_fist_stage:
            self.firts_stage_model = FirstStageModel(use_make_noisy =self.use_make_noisy)
        ...
        self.text_branch = create_latent_pann_model(latent_cfg, enable_fusion, fusion_type)
    ...
    def encode_text(self, text, device):
        ...
        x = self.text_branch(text.to(device=device, non_blocking=True))
        x = self.text_projection(x["embedding"])
        return x

    def get_latent_input(self, audio, text):
        if self.use_fist_stage:
            if text is not None and audio is not None:
                fbank = audio["fbank"]
                text = self.firts_stage_model(fbank)
            elif text is not None and audio is None:
                text = self.firts_stage_model(text)
        else:
            pass
    ...
    def forward(self, audio, text, device=None):
        ...
        # if text is not None:
        #     text = self.get_latent_input(text)

        # make it compatible with CLAP training pipeline
        text = self.get_latent_input(audio, text)
