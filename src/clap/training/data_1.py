...
from clap.training.get_latent_data import Latent_data_getter
...
class WavCapsdataset(Dataset):
    def __init__(self, input_filename, transforms, args = None):
        logging.debug(f"Loading csv data from {input_filename}.")

        self.args = args

        # if needed to get latent getter function that gets first stage embbedings from VAE model
        if "latent" in args.amodel:
            # import torch.multiprocessing as mp
            # mp.set_start_method('spawn', force=True)
            self.Latent_getter = Latent_data_getter()
        else:
            self.Latent_getter = None
    ...
    def read_datafile(self, file_path): # split):
        ...
        with open(file_path, "r") as fp:
            ...
            dataset_name = os.path.basename(json_directory)

            flac_directory = os.path.abspath(os.path.join(json_directory, '..', '..', 'flac', dataset_name))
            ...
            for entry in data:
                # get flac data pathe
                entry_id = entry['id']
                if entry_id.endswith(".wav"):
                    entry_id = entry_id[:-4]
                flac_file_path = os.path.join(flac_directory, entry_id + '.flac')
                entry['wav'] = flac_file_path

                entry['frame_offset'] = 0

                # get data category if given
                labels = entry.get('category',  'not available')
    ...
    def __getitem__(self, idx):
        ...
        if self.Latent_getter is not None:
            fbank, _, _, _  = self.Latent_getter._wav2fbank(audio_data, orig_sr)
            sample["fbank"] = fbank.unsqueeze(0).cpu()
...
class Dcase_Dataset(WavCapsdataset):
    def __init__(self, input_filename, transforms, args = None):
        super().__init__(input_filename, transforms, args)

    def read_datafile(self, path) -> list:

        from typing import List
        
        self.args.translator: dict = {
                            "DogBark": "dog bark",
                            "Footstep": "footstep",
                            "GunShot": "gunshot",
                            "Keyboard": "keyboard",
                            "MovingMotorVehicle": "moving motor vehicle",
                            "Rain": "rain",
                            "Sneeze_Cough": "sneeze or cough",
                            }
        ...
        #         if file_path == "Sneeze":
        #             file_path = "Sneeze_Cough"   ### idiots!!!                
        #         wav_path = os.path.join(os.path.dirname(path), file_path)
                
        #         cat = row['category']
        #         if cat == "Sneeze":
        #             cat = "Sneeze_Cough"   ### idiots!!!
        #         category = [cat]

        #         text = clas_dict[cat]
        #         caption = f"This is a sound of {text}"
        ...
        #             "caption": caption,
        #         })
        for root_dir, _, file_list in os.walk(os.path.dirname(path)):
            for file_name in file_list:
                if os.path.splitext(file_name)[-1] == ".wav":
                    # label = clas_dict[root_dir.split("/")[-1]]
                    label = os.path.basename(root_dir)
                    caption = f"This is a sound of {self.args.translator[label]}"
                    training_files.append(
                        {
                            "labels": [label],
                            "wav": f"{root_dir}/{file_name}",
                            "frame_offset": 0,
                            "caption": caption, 
                        }
                    )
        ...
        class_lables : dict =  {
                                    ...
                                    # "Gun Shot": 2,
                                    # "Keyboard": 3,
                                    # "Moving Motor Vehicle": 4,
                                    # "Rain": 5,
                                    # "Sneeze Cough": 6,
                                             
                                }
        self.args.class_index_dict = class_lables
    ...
    def __getitem__(self, idx):
        ...
        sample = {}
        audio_data, orig_sr =torchaudio.load(self.images[idx], frame_offset =  self.frame_offset[idx], num_frames = 220500)
...
class CsvDataset(Dataset):
    def __init__(self, input_filename, transforms, args = None):
        ...
        class_index_dict=copy.deepcopy(args.class_index_dict)
        self.mid_mapping_dict = dict(zip(class_index_dict['mid'], class_index_dict[['index', 'display_name']].to_records(index=False)))
    ...
    def __getitem__(self, idx):
        ...
        sample = {}
        audio_data, orig_sr =torchaudio.load(self.images[idx])
        ...
        machine_codes = str(self.captions[idx])

        sample["machine_codes"] = machine_codes



        if self.mid_mapping_dict is not None:
            # https://stackoverflow.com/questions/48004243/how-to-share-large-read-only-dictionary-list-across-processes-in-multiprocessing
            # https://stackoverflow.com/questions/45693949/storing-strings-in-a-multiprocessing-sharedctypes-array
            # key, val = class_index_dict
            # key = key[:].split('\n')
            # _dict = {k: v for k, v in zip(key, val)}
            sample["class_label"] = np.zeros(len(self.mid_mapping_dict.keys()))
            text = ""
            text_list = machine_codes.split(',')
            for x in text_list:
                if x in self.mid_mapping_dict:
                    index, display_name = self.mid_mapping_dict[x]
                sample["class_label"][index] = 1
                text +=  f"{display_name} " 
...
def get_Dcase_Dataset(args, preprocess_fn, is_train):
    input_filename = args.train_data if is_train else args.val_data
    assert input_filename

    dataset = Dcase_Dataset(
        input_filename,
        preprocess_fn,
        args=args
    )    
    ...
    if args.val_data[0] is not None: 
        train_data, val_data = torch.utils.data.random_split(dataset, [int(0.9 * num_samples), int(0.1 * num_samples)], generator=torch.Generator().manual_seed(42))
        if is_train:
            dataset = train_data
        else:
            dataset = val_data
...
def get_WavCaps_dataset(args, preprocess_fn, is_train):
    input_filename = args.train_data if is_train else args.val_data
    assert input_filename
...
def get_dataset_fn(data_path, dataset_type):
    ...
    elif dataset_type == "WavCaps":
        return get_WavCaps_dataset
    elif dataset_type == "Dcase":
        return get_Dcase_Dataset
    else:
...
def get_data(args, model_cfg):
    ...
    if args.dataset_type == "Dcase":
        args.train_data = [args.datasetpath]
        if "valid" in args.datasetinfos:
            args.val_data = [args.datasetpath]
            args.val_dataset_names = "Dcase"
